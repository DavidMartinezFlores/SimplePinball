...
def ballWallImpactSound():#---ONLY SOUND
    ballWall=mixer.Sound('sources/ballBlockCollission.mp3')
    ballWall.play()
...
def updateball(moveLeftTunnel,moveLeftTunnel2,moveLeftTunnel3,moveRigthTunnel,moveRigthTunnel2,moveRigthTunnel3,extraLife_bumper_X,lifes,ball,ball2,ball3,external_wall_Btunnel,internal_wallBtunnel,extraLife_bumper,Walls,WallslateralStick_wall,topWall,whiteBumpers,LeftStick,RigthStick,paloExterior,red_closer,points,cont):
    ...
    for Wall in impacts_list_rigth_Stick2:
        ...
        if keyPressed[pygame.K_RETURN]:
            ...
            ball2.speed_y=-13+-cont
            if ball2.speed_x <0:
                ball2.speed_x=-5+-cont
            else:
                ball2.speed_x=5+cont
        else:
            ball2.rect.bottom=Wall.rect.top
    ...
    for Wall in impacts_list_rigth_Stick3:
        ...
        if keyPressed[pygame.K_RETURN]:
            ...
            ball3.speed_y=-13+-cont
            if ball3.speed_x <0:
                ball3.speed_x=-5+-cont
            else:
                ball3.speed_x=5+cont
        else:
            ball3.rect.bottom=Wall.rect.top
...
def LeftStick(screen,spriteLeftStick):
    ...
    if keyPressed[pygame.K_SPACE]:
        #ROTATE TO +0
        new_image= pygame.image.load("images/flipperLeftUp3.png").convert_alpha()
    else:
        #ROTATE TO -25
        new_image= pygame.image.load("images/flipperLeftDown3.png").convert_alpha()
...
def RigthStick(screen,spriteRigthStick):
    ...
    if keyPressed[pygame.K_RETURN]:
        #ROTATE TO +0
        new_image= pygame.image.load("images/flipperRigthUp3.png").convert_alpha()
    else:
        #ROTATE TO -25
        new_image= pygame.image.load("images/flipperRigthDown3.png").convert_alpha()
...
def makeExtraLifeBumper(sprite_extraLife_bumper, x, y):
    ...
    extraLife_bumper.image = pygame.image.load("images/bumperPAppears.png").convert_alpha()
    #Add to the sprite group
...
def makeExternalWallsBigTunnel(sprite_externalWall_Btunnel):
    ...
    #Wall 28
    Wall28 = makeExternalWallBTunnel(380,380,160,10)#top
    sprite_externalWall_Btunnel.add(Wall28)
    #wall 29
    Wall29 = makeExternalWallBTunnel(380,420,160,10)#bottom
...
def ballSpeedLimits():
    ...
    #SPEED LIMITS-------------
    if ball.speed_x>=10:#BALLS X LIMITS POSITIVE
        ball.speed_x=10
    if ball2.speed_x>=10:
        ball2.speed_x=10
    if ball3.speed_x>=10:
    ...
    if ball.speed_x<=-10:#BALLS X LIMITS NEGATIVE
        ball.speed_x=-10
    if ball2.speed_x<=-10:
        ball2.speed_x=-10
    if ball3.speed_x<=-10:
    ...
    if ball.speed_y>20:#BALLS Y LIMITS POSITIVE
        ball.speed_y=20
    if ball2.speed_y>20:
        ball2.speed_y=20
    if ball3.speed_y>20:
...
def ballsAppears(old_points2):
    ...
    if points%500==0 and points>0 and not(old_points2==points) and ball2.rect.y>SCREEN_HEIGHT and ball3.rect.y>SCREEN_HEIGHT:
        ...
        ball2.rect.x=350
        ball2.rect.y=100
        ball2.speed_y=-2#SPEED Y
        ball2.speed_x=6#SPEED X

        ball3.rect.x=560
        ball3.rect.y=100
        ball3.speed_y=-2#SPEED Y
        ball3.speed_x=-6#SPEED X
        ballsAppearsSound()
    return old_points2
...
def initialSound(startGame):
    ...
    if startGame:
        inicioP=mixer.Sound('sources/partyStart.mp3')
        inicioP.play(0)
        startGame=False
    return startGame
...
def closedSound():
        cerRED=mixer.Sound('sources/close.mp3')
        cerRED.play(1)       
        cerRED.set_volume(4)
